...
public class ServiciosController extends Controller implements IcrudViewsHandler {
  ...
  public void index(Context context) {
    ...
    List servicios = todos.stream().filter(s ->
        !comunidad.getServiciosDeInteres().contains(s)).toList();
  ...
  public void show(Context context) {
    RepositorioServicios.getInstance().clean();
    List<Incidente> incidentes = RepositorioIncidentes.getInstance()
        .incidentesDeServicioYcomunidad(Long.parseLong(context.pathParam("idServicio")),
            Long.parseLong(context.pathParam("id")));
    List<Incidente> incidentesResueltos = incidentes.stream().filter(incidente ->
        incidente.getEstaResuelto() == true).collect(Collectors.toList());
    List<Incidente> incidentesAbiertos = incidentes.stream().filter(incidente ->
        incidente.getEstaResuelto() == false).collect(Collectors.toList());
    Servicio servicio = RepositorioServicios.getInstance()
        .get(Long.parseLong(context.pathParam("idServicio")));
  ...
  public void save(Context context) {
    Servicio servicio = new Servicio(context.formParam("nombreServicio"),
        ...
        RepositorioEstablecimiento.getInstance()
            .get(Long.parseLong(context.formParam("establecimiento"))));
